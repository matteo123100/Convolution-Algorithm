// Codice Ottimizzato Con Affinity
#include <iostream>
#include <vector>
#include <thread>
#include <random>
#include <fstream>
#include <chrono>
#include <string>
#include <windows.h>

#define ONE_THREAD_EXECUTION false

using namespace std::chrono;

const int KERNEL_SIZE = 3;
const int COLS = 4096;
const int ROWS = 2160;

//int** inputMatrix = new int* [ROWS];
//int** outputMatrix = new int* [ROWS - KERNEL_SIZE + 1];
int* inputMatrix = new int[ROWS * COLS];
int* outputMatrix = new int[(ROWS - KERNEL_SIZE + 1) * (COLS - KERNEL_SIZE + 1)];

//void convolutionRows(const int kernel[KERNEL_SIZE][KERNEL_SIZE], int startRow, int endRow)
void convolutionRows(const int kernel[KERNEL_SIZE], int startRow, int endRow)
{
    int outputRowSize = ROWS - KERNEL_SIZE + 1; // 1920 - 2 + 1 --> 1919
    int outputColSize = COLS - KERNEL_SIZE + 1; // 1080 - 2 + 1 --> 1079
    int value;
    for (int row = startRow; row < endRow; row++)  // row � [0,1919]
    {
        for (int col = 0; col < outputColSize; col++) // col � [0,1079]
        {
            value = 0;
            for (int kRow = 0; kRow < KERNEL_SIZE; kRow++) // kRow € [0,1]
            {
                int x1 = (row + kRow) * COLS + col;
                int x2 = kRow * KERNEL_SIZE;

                for (int kCol = 0; kCol < KERNEL_SIZE; kCol++) // kCol € [0,1]
                {
                    //value += inputMatrix[row + kRow][col + kCol] * kernel[kRow][kCol];
                    
                    value += inputMatrix[ x1 + (kCol)] * kernel[x2 + kCol];
                }
            }
            //std::cout << "rows: " << row << "\tcols: " << col << "\n";
            //outputMatrix[row][col] = value;
             outputMatrix[row * outputColSize + col] = value;              
        }
    }
}

//void convolution(const int kernel[KERNEL_SIZE][KERNEL_SIZE], int N_THREAD)
void convolution(const int kernel[KERNEL_SIZE], int N_THREAD)
{
    int rowsPerThread = ROWS / N_THREAD;

    //   std::vector<std::thread> threads;

    std::thread threads[8]; //<-----------------------------------------------------------------------------------
    // Create an array to store thread handles
    //HANDLE threadHandles[MAXIMUM_PROCESSORS]; <<
    int x[] = {0,2,4,6,1,3,5,7,8,10,12,9};
    for (int i = 0; i < N_THREAD; ++i)
    {
        int startRow = i * rowsPerThread;

        int endRow = (i == N_THREAD - 1) ? (ROWS - KERNEL_SIZE + 1) : (startRow + rowsPerThread);
        // threads.emplace_back(convolutionRows, std::cref(kernel), startRow, endRow);
   //     threadHandles[i] = CreateThread(NULL, 0, ThreadFunction, &threadId, CREATE_SUSPENDED, NULL);   <<
        threads[i] = std::thread{ convolutionRows,kernel, startRow, endRow };
        DWORD_PTR affinityMask = 1ULL << (x[i]%8);

        if (!SetThreadAffinityMask(threads[i].native_handle(), affinityMask)) {
            std::cerr << "Error: Unable to set thread affinity\n";
        }

    }

    for (auto& thread : threads) // Attende la fine di tutti i thread
    {
        thread.join();
    }
}

void stampaMatrix() {
    for (int i = 0; i < (ROWS - KERNEL_SIZE + 1); i++) {
        for (int j = 0; j < (COLS - KERNEL_SIZE + 1); j++)
            //std::cout << outputMatrix[i][j] << "\t";
            std::cout << outputMatrix[i * (COLS - KERNEL_SIZE + 1) + j] << "\t";
        std::cout << std::endl;
    }
}

int main(int argc, char* argv[])
{
    /*
    for (int i = 0; i < ROWS; ++i)
    {
        inputMatrix[i] = new int[COLS]; // Alloca un array di interi per ogni riga
        if (inputMatrix[i] == nullptr)
        {
            std::cout << i << "dimensione insufficiente\n";
            return 0;
        }
    }

    int colOutputSize = COLS - KERNEL_SIZE + 1;
    for (int i = 0; i < colOutputSize; ++i)
    {
        outputMatrix[i] = new int[colOutputSize]; // Alloca un array di interi per ogni riga
        if (outputMatrix[i] == nullptr)
        {
            std::cout << i << "dimensione insufficiente\n";
            return 0;
        }
    }
    */

    int N_THREAD = 8; //<-----------------------------------------------------------------------------------
    
    if (!ONE_THREAD_EXECUTION)
    {
        if (argc > 1)
            N_THREAD = std::stoi(argv[1]);
       
    }

    // ---------------------------------  INPUT MATRIX INITIALIZATION ---------------------------------
    int initialValue = 1;
    for (int i = 0; i < ROWS; ++i)
    {
        for (int j = 0; j < COLS; ++j)
        {
            //inputMatrix[i][j] = initialValue; //distribution(gen);
            inputMatrix[i * COLS + j] = initialValue; //distribution(gen);
            initialValue++;
            //std::cout << inputMatrix[i][j] << " ";
        }
        //std::cout << std::endl;
    }
    // ------------------------------------------------------------------------------------------------

    // --------------------------------- KERNEL MATRIX INITIALIZATION ---------------------------------
    initialValue = 1;
    //int kernel[KERNEL_SIZE][KERNEL_SIZE];
    int kernel[KERNEL_SIZE * KERNEL_SIZE];
    for (int i = 0; i < KERNEL_SIZE; ++i)
    {
        for (int j = 0; j < KERNEL_SIZE; ++j)
        {
            //kernel[i][j] = initialValue; // distribution(gen);
            kernel[i * KERNEL_SIZE + j] = initialValue; // distribution(gen);
            initialValue++;
            //std::cout << kernel[i][j] << " ";
        }
        // std::cout << std::endl;
    }
    // ------------------------------------------------------------------------------------------------

    //HANDLE threadHandles[8];



    auto begin = steady_clock::now(); // ***** TIMER START


    //if (ONE_THREAD_EXECUTION)
    //    convolutionRows(kernel, 0, (ROWS - KERNEL_SIZE + 1));
    //else
    convolution(kernel, N_THREAD);


    auto end = steady_clock::now();   // ***** TIMER STOP


    //stampaMatrix();

    // ----------------------------------------- WRITE DATA ON FILE -------------------------------------

    auto elapsed_ms = duration_cast<milliseconds>(end - begin);
    std::cout << "Tempo di esecuzione: " << elapsed_ms.count() << " ms" << std::endl;

    std::string s = "4k_" + std::to_string(N_THREAD) + ".txt";
    std::ofstream fout(s, std::ios::app);
    fout << elapsed_ms.count() << "\n";
    fout.close();
    // ---------------------------------------------------------------------------------------------------

    //system("pause");
    return 0;
}


/*
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<int> distribution(0, 9); // Valori compresi tra 0 e 9
    */